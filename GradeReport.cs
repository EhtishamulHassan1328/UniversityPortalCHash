...
namespace checking
{
    public partial class GradeReport : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            string query = $"SELECT * FROM Grades WHERE CourseName = @CourseName";
            ...
            using (SqlConnection conn = new SqlConnection("Data Source=DESKTOP-GG6QOPE;Initial Catalog=FlexDB;Integrated Security=True"))
            {
                ...
                using (SqlCommand cmd = new SqlCommand(query, conn))
                {
                    ...
                    using (SqlDataAdapter adapter = new SqlDataAdapter(cmd))
                    {
                        ...
                        if (dt.Rows.Count == 0)
                        {
                            label3.Text = $"No grade record found for {courseName}.";
                        ...
                        else
                        {
                            ...
                            dataGridView1.Columns["GradeId"].HeaderText = "ID";
